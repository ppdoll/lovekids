...
/**
 * 숫자 뒤에 붙는 조사. hasJong은 한글 음절만 보므로 "5"에 쓰면 항상 받침이 없다고 나온다.
 * 그래서 숫자를 소리내어 읽었을 때의 끝소리로 정한다 ("3을"=삼을, "5를"=오를).
 * 0으로 끝나는 수는 십·백·천으로 읽어 모두 받침이 있고, 0 자체도 "영"이라 받침이 있다.
 */
const 숫자받침 = (n: number) => [0, 1, 3, 6, 7, 8].includes(Math.abs(n) % 10);
const 숫자을를 = (n: number) => `${n}${숫자받침(n) ? "을" : "를"}`;
...
const middleGens: Gen[] = [
  ...
  () => {
    ...
    const x = ri(-12, 20) || 7;
    const a = ri(2, 9);
    // 0이면 "0을 더하면"이 되어 문제가 성립하지 않으므로 0은 뽑지 않는다
    const b = ri(-15, 15) || 6;
    return short(
      `어떤 수를 ${a}배 한 뒤 ${b > 0 ? `${숫자을를(b)} 더하면` : `${숫자을를(-b)} 빼면`} ${a * x + b}이 됩니다. 어떤 수는 얼마인가요?`,
      [`${x}`],
  ...
  () => {
    ...
    return short(
      `서로 다른 ${items[0]} ${a}가지와 ${items[1]} ${b}가지가 있습니다. 하나씩 골라 만들 수 있는 ${은는(items[2])} 모두 몇 가지인가요?`,
      withUnit(a * b, "가지"),
      `${a} × ${b} = ${a * b}`,
    );
  },
